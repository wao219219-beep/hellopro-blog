...
async function load(show=true,append=false){
 ...
 if(append) state.loadingMore=true; else state.loading=show;
 render();
 try{
  const oldIds=new Set(state.posts.map(x=>x.id));
  const offset=append?state.posts.length:0;
  const r=await fetch(queryFor(offset),{cache:'no-store'});
  ...
  state.banner='ブログ取得に失敗しました。しばらくしてから再読み込みしてください。';
 }
